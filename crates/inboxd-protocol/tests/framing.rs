...
fn frozen_method_surface_and_additive_capability_event_are_exact() {
    ...
    assert_eq!(LEGACY_REQUEST_METHODS[18..], ["settings.get", "settings.update", "subscribe"]);
...
fn json_lines_preserve_fragmented_utf8_and_enforce_raw_byte_bounds() {
    ...
    assert!(oversized.push(b"123456789").unwrap_err().to_string().contains("8"));
}
