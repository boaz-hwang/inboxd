...
fn request_parser_preserves_roles_handshake_and_unsupported_settings() {
    let hello = json!({"type":"request","id":"h","method":"system.hello","params":{"role":"reader"}});
    assert_eq!(parse_request(&hello, None).unwrap().method, "system.hello");
    let denied = json!({"type":"request","id":"p","method":"safety.intent.listPending","params":{}});
    assert_eq!(parse_request(&denied, Some(ClientRole::Agent)).unwrap_err().code, "BAD_REQUEST");
    for method in ["settings.get", "settings.update", "capability.list"] {
        let request = json!({"type":"request","id":"x","method":method,"params":{}});
        assert_eq!(parse_request(&request, Some(ClientRole::Reader)).unwrap().method, method);
    }
    let unknown = json!({"type":"request","id":"x","method":"send.now","params":{}});
    assert!(matches!(parse_request(&unknown, None), Err(ProtocolError { .. })));
}
...
fn worker_frames_are_raw_bounded_and_correlated() {
    ...
    assert_eq!(validate_worker_request_frame(&request_line).unwrap(), *request);
    let response_line = serde_json::to_vec(response).unwrap();
    assert_eq!(validate_worker_response_frame(&response_line, request).unwrap(), *response);
    ...
    assert!(validate_worker_response_frame(&serde_json::to_vec(&ambiguous).unwrap(), request).is_err());
    ambiguous["error"]["retryable"] = json!(false);
    assert!(validate_worker_response_frame(&serde_json::to_vec(&ambiguous).unwrap(), request).is_ok());
}
