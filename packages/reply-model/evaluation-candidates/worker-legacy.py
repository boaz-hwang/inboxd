...
읽음 여부와 관계없이 상대의 최근 실제 발언에 내가 보낼 다음 메시지를 작성하세요. 상대의 말을 복사하거나 상대 역할로 답하지 마세요.
...
class ReplyWorker:
    ...
    def create_reply(self, request, compiled, omitted):
        ...
        if payload["conversation"] and payload["conversation"][-1]["author_role"] == "self":
            return {"status": "abstained", "text": None, "error": "no_reply_target"}
        ...
        generation_input = [compiled[0]] + [
            ...
            **{key: value for key, value in payload.items() if key != "conversation"},
            "task": ("상대가 말한 사실만 짧게 받아들이고, 내가 답을 모르는 사항은 확인이 필요하다고 표현하세요. 상대가 내게 물은 질문을 상대에게 그대로 되묻지 마세요. 근거 없는 일정·계약 확정이나 이미 수행했다는 주장을 추가하지 마세요."
    ...
    def handle(self, request):
        ...
        try:
            ...
            if isinstance(request.get("context"), list) and request["context"]:
                ...
                conversation = json.loads(compiled[-1]["content"])["conversation"]
                # The latest actual human utterance is the reply target. A sent
                # message is context for future replies, never a new target itself.
                if conversation and conversation[-1]["author_role"] == "self":
                    result.update(status="abstained", error="no_reply_target")
                    return result
            ...
            if operation == "decide":
                ...
                if generated.get("status") == "abstained" and generated.get("error") != "no_reply_target":
